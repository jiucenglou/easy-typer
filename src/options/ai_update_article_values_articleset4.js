...
// 读取 SQL 文件
try {
  const sqlFilePath = path.join('scripts', 'sqls', 'articleSet4.sql');
  const sqlContent = fs.readFileSync(sqlFilePath, 'utf8');
  
  // 使用正则表达式提取文章 ID 和内容
  const regex = /\(8, '(articleSet4-\d+)', '[^']*', "([^"]+)"/g;
  const articleMap = {};
  ...
  // 读取 TypeScript 文件
  const tsFilePath = path.join('src', 'options', 'articleSet4.ts');
