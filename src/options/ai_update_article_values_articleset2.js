...
// 读取 SQL 文件
try {
  const sqlFilePath = path.join('scripts', 'sqls', 'articleSet2.sql');
  const sqlContent = fs.readFileSync(sqlFilePath, 'utf8');
  
  // 使用正则表达式提取文章 ID 和内容
  // 注意：这里修改了正则表达式以匹配 articleSet2 的格式
  const regex = /\(6, '(articleSet2-\d+)', '[^']*', "([^"]+)"/g;
  const articleMap = {};
  ...
  // 读取 TypeScript 文件
  const tsFilePath = path.join('src', 'options', 'articleSet2.ts');
  ...
  console.log('articleSet2.ts 文章内容已成功更新！');
} catch (error) {
